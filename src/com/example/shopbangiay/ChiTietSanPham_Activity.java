...
import android.widget.ListAdapter;
	...
	public class ChiTietSanPham_Activity extends Activity {
	    ...
	    private void hienThiSanPham() {
	        DocAnh.hienThiAnh(this, sanPham.getImageURL(), imgSanPham);
	        ...
	        btnThemGio.setOnClickListener(new View.OnClickListener() {
	            ...
	            public void onClick(View v) {
	                if (sanPham.getSoLuong() <= 0) {
	                    Toast.makeText(ChiTietSanPham_Activity.this, "Hết hàng!", Toast.LENGTH_SHORT).show();
	                    return;
	                }
	                GioHang.getInstance().themSanPham(sanPham, 1);
	                Toast.makeText(ChiTietSanPham_Activity.this, "Đã thêm vào giỏ hàng!", Toast.LENGTH_SHORT).show();
	            }
	        });
	        ...
	        btnMuaNgay.setOnClickListener(new View.OnClickListener() {
	            ...
	            public void onClick(View v) {
	                ...
	                GioHang.getInstance().themSanPham(sanPham, 1);
	                startActivity(new Intent(ChiTietSanPham_Activity.this, GioHang_Activity.class));
	            }
	        });
	    }
	    ...
	    private void taiDanhGia(Connection con) throws Exception {
	        ...
	        List<DanhGia> dsDanhGia = new ArrayList<DanhGia>();
	        while (rs.next()) {
            ...
            capNhatChieuCaoDanhGia();
	    }
	 
	    private void toggleYeuThich() {
	        if (NguoiDungHienTai_Activity.nguoiDungHienTai == null) {
                Toast.makeText(this, "Vui long dang nhap de su dung yeu thich.", Toast.LENGTH_SHORT).show();
                return;
            }
        ...
        private void capNhatChieuCaoDanhGia() {
            ListAdapter adapter = lvDanhGia.getAdapter();
            if (adapter == null) {
                return;
            }

            int tongChieuCao = 0;
            int chieuRongDo = View.MeasureSpec.makeMeasureSpec(lvDanhGia.getWidth(), View.MeasureSpec.AT_MOST);
            for (int i = 0; i < adapter.getCount(); i++) {
                View item = adapter.getView(i, null, lvDanhGia);
                item.measure(chieuRongDo, View.MeasureSpec.UNSPECIFIED);
                tongChieuCao += item.getMeasuredHeight();
            }

            android.view.ViewGroup.LayoutParams params = lvDanhGia.getLayoutParams();
            params.height = tongChieuCao + (lvDanhGia.getDividerHeight() * Math.max(adapter.getCount() - 1, 0));
            lvDanhGia.setLayoutParams(params);
            lvDanhGia.requestLayout();
        }
